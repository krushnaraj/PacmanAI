...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    #find the closest ghost position for each ghost with respect to its best belief
    overall = list()
    for x in range(len(livingGhostPositionDistributions)):
      if 1:
        ghostdistribution = livingGhostPositionDistributions[x]
        bestprob = 0.000000
        for pos,prob in ghostdistribution.items():
          if prob > bestprob:
            bestprob = prob
            bestpos = pos
        overall.append(bestpos)


    #find the destination positon we plan to target based on he distance
    shortestdist = 100000

    for n in range(len(overall)):
      distancetoguage = self.distancer.getDistance(pacmanPosition, overall[n])
      if distancetoguage < shortestdist:
        shortestdist = distancetoguage
        gotopos = overall[n]

    #choose action based on successor position
    finaldist = 10000
    for action in legal:
      successorPosition = Actions.getSuccessor(pacmanPosition, action)
      successordist = self.distancer.getDistance(successorPosition, gotopos)
      if successordist<finaldist:
        finaldist = successordist
        bestaction = action
    return bestaction
