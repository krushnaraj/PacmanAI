...
class ExactInference(InferenceModule):
  ...
  def elapseTime(self, gameState):
    ...
    beliefupdater = util.Counter()
    #noisydistance = gameState.getNoisyGhostDistances()
    for p in self.legalPositions:
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
      prevbelief = self.beliefs[p]
      for newPos, prob in newPosDist.items():
        beliefupdater[newPos] = beliefupdater[newPos] + prob*prevbelief
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    n = self.numParticles       #300
    legal = self.legalPositions #216
    uniformInit = []            #play with 300 different particles in 216 positions
    #print len(legal)
    #util.raiseNotDefined()
    ...
    if n > lleg:
        for x in range(n):
            uniformInit.append(legal[x % lleg])
    else:
        for x in range(n):
            uniformInit.append(legal[x])   #fill up as many positions as possible
    #print uniformInit
    #print len(uniformInit)
    #print len(legal)
    #print legal
    #util.raiseNotDefined()
  ...
  def getBeliefDistribution(self):
    ...
    for pos in self.particles:
        beliefdistribution[pos] = beliefdistribution[pos] + 1
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
        ghostPositions = []
        for j in range(self.numGhosts):         #for each particle assign four ghostpositions
            randomPos = random.choice(self.legalPositions)
            ghostPositions.append(randomPos)
        self.particles.append(tuple(ghostPositions))      #append each of these sets of ghost positions to the total particle set
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      for i in range(self.numGhosts):
          disttonewpos = getPositionDistributionForGhost(setGhostPositions(gameState, oldpos),i, self.ghostAgents[i])
          newParticle[i] = util.sample(disttonewpos)
